...
# ret = device.read_byte_data(I2C_ADDRESS, 0xD0)
# print(ret)
...
class BME280:
    def __init__(self, address, i2cbus: Optional[SMBus] = None):
        if i2cbus is None:
            self.i2cbus = SMBus(1)
        else:
            self.i2cbus = i2cbus
    ...
    def _read(self, *cmd) -> int:
        res = 0
        for c in cmd[::-1]:
            res = res << 8 | self.i2cbus.read_byte_data(self.address, c)
        return res

    def _read_bytes(self, *cmd) -> bytes:
        res = b""
        for c in cmd[::-1]:
            res += struct.pack("B", self.i2cbus.read_byte_data(self.address, c))
        return res

    def _read_block(self, *cmd) -> list[int]:
        res = []
        for c in cmd:
            res.append(self.i2cbus.read_byte_data(self.address, c))
        return res
    ...
    def set_mode(self, mode: Mode):
        ...
        self.i2cbus.write_byte_data(self.address, 0xF4, current_ctrl_meas & (~0b11) | mode.value)
...
if __name__ == '__main__':
    device = SMBus(1)
    bme280 = BME280(I2C_ADDRESS, i2cbus=device)
    ...
    while True:
        try:
            ...
            pressure = 0
            t = time.time()
            print(f"Time: {t - start_time:0.2f}, Temperature: {temp}, Pressure: {pressure}")
            print(bme280._adc_T)
